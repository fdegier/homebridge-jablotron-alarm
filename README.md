...
- **contact sensor**: this is an contact sensor (PGM device) connected to Jablotron unit
...
- 1 contact sensor that provides update of living room windows open/close status
- 1 thermometer
...
- **reversed_status**: optional, used by contact sensors only. By default contact sensor is opened when PGM is set and closed when unset. With this option set to true
the logic is reversed ie contact sensor is closed when PGM is set and opened when set

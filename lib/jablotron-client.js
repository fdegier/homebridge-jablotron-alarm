...
JablotronClient.prototype = {
    containsErrorCode: function (response ) {
        console.log("containsErrorCode: " + JSON.stringify(response));
        let errors = [];
        if (response['data']['control-errors']) {
            errors = response['data']['control-errors'];
            // console.log("JABLOTRON: Control error code: " + JSON.stringify(response['data']['control-errors'][0]));
        ...
        console.log("JABLOTRON: Error code: " + JSON.stringify(errors));
        return errors;
    },

    matchesErrorCode: function (response, errorCode = undefined) {
        // Errors can be in two keys: 'errors' and 'control-errors'
        console.log("matchesErrorCode: " + JSON.stringify(response));
        let errors = [];
        if (response['data']['control-errors']) {
            errors = response['data']['control-errors'];
            console.log("JABLOTRON: Control error code: " + JSON.stringify(response['data']['control-errors']));
        } else if (response['errors']) {
            errors = response['errors'];
            console.log("JABLOTRON: Error code: " + response['errors']);
        } else {

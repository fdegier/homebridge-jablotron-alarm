...
Jablotron.prototype = {
        ...
        this.fetchSessionId(function (sessionId) {
            if (sessionId) {
                self.client.doAuthenticatedRequest("/" + this.service_type + '/sectionsGet.json', payload, sessionId, function (response) {
                    ...
                    console.log("JABLOTRON: Sections: " + JSON.stringify(sections, null, 2));

                    callback(sections);
                }, function (error) {
                    if (self.tryHandleError(error)) {
                        self.getSections(callback);
                    } else {
                        callback(null);
                    }
                });
                ...
                self.client.doAuthenticatedRequest('/' + this.service_type + '/sectionsGet.json', payload, sessionId, function (response) {
                    ...
                    if (responseData != null && responseData.length > 0) {
                        ...
                        segments.forEach(function (segment) {
                            let segmentKey = segment['cloud-component-id'];

                            if (segmentKey === accessory.getSegmentId()) {
            ...
            if (sessionId) {
                ...
                self.client.doAuthenticatedRequest('/' + "JA100" + '/sectionsGet.json', payload, sessionId, function (response) {
                    ...
                    if (self.tryHandleError(error)) {
                        ...
                        console.log("JABLOTRON: getAccessoryStates error");
                        callback(null, null, null);
                    }
                });
            } else {
    ...
    changeAccessoryState: function (accessory, state, callback) {
        ...
        let keyboardOrSegmentKey = accessory.getSegmentId();
        if (JablotronConstants.isJablotronPartiallyArmed(state) && accessory.isPartiallyArmedAvailable()) {
            keyboardOrSegmentKey = accessory.getSegmentId();
        }
            ...
            'control-components': [
                {
                    ...
                    'component-id': accessory.getSegmentId()
        ...
        this.service.log("Switching section " + accessory.getSegmentId() + " (using " + keyboardOrSegmentKey + ") to new state: " + state);

        ...
        this.fetchSessionId(function (sessionId) {
            if (sessionId && sessionId != null) {
                self.client.doAuthenticatedRequest('/dataUpdate.json', payload, sessionId, function (response) {
                    ...
                    console.log(responseData["service_events_type"]);
                    if (responseData["data"] != null) {
                        if (responseData["service_events_type"] == "tamper" || responseData["service_events_type"] == "failue") {
                            callback("triggered")
                        }
                ...
                self.client.doAuthenticatedRequest('/dataUpdate.json', payload, sessionId, function (response) {
                    ...
                    if (responseData["data"] != null) {
                        ...
                        if (responseData["data"].length > 1) {
                            segments = responseData["data"][1]['data']['segments'];
                            segments.forEach(function (segment) {
                                callback(segment['segment_key'], segment['segment_state']);
                            });
                        }
